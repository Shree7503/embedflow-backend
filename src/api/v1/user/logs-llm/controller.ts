...
export const listQueryLogs: RequestHandler = async (req: LogRequest, res: Response, next: NextFunction) => {
    ...
    if (startDate) {
        ...
        if (isNaN(date.getTime())) return next(new AppError('Invalid startDate format.'));
        filters.startDate = date;
    }

    if (endDate) {
        const date = new Date(endDate);
        if (isNaN(date.getTime())) return next(new AppError('Invalid endDate format.'));

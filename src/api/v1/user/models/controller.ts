...
interface UserContext { id: string; username: string; email: string };
...
export const createModel = async (req: ModelRequest, res: Response, next: NextFunction) => {
    ...
    if (!Object.values(ModelType).includes(modelType)) {
         return next(new AppError(`Invalid model type. Must be one of: ${Object.values(ModelType).join(', ')}.`));
    }

    try {
        const model = await modelService.createModel(userId, { modelName, modelType, description });
        res.status(201).json({ 
...
export const listModels = async (req: ModelRequest, res: Response, next: NextFunction) => {
...
export const deleteModel = async (req: ModelRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
        return next(new AppError('Unauthorized: User context missing.'));
    }

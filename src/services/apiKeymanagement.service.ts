...
export class ApiKeyManagementService {
    ...
    public async generateKey(userId: string, projectId: string): Promise<GeneratedApiKey> {
        ...
        if (!project || project.userId !== userId) {
            throw new AppError('Access denied: Project not found or does not belong to user.');
        }
    ...
    public async getKeys(userId: string, projectId: string): Promise<Partial<ApiKey>[]> {
    ...
    public async revokeKey(userId: string, keyId: string): Promise<Partial<ApiKey>> {

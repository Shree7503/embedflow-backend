...
export class ModelService {
    ...
    public async createModel(userId: string, data: CreateModelParams): Promise<Model> {
        ...
        if (existingModel) {
            throw new AppError(`Model name "${data.modelName}" already exists for this user.`);
        }
    ...
    public async deleteModel(userId: string, modelId: string): Promise<Model> {
        ...
        if (usedByProject) {
            throw new AppError( `Model is currently assigned to project "${usedByProject.name}". Please reassign the project models before deleting.`);
        }
    ...
    public async updateProjectModels({ projectId, userId, retrievalModelId, generatorModelId }: UpdateProjectModelsParams): Promise<Project> {
        ...
        if (modelsToVerify.length > 0) {
            ...
            if (models.length !== modelsToVerify.length) {
                 throw new AppError( 'One or more specified models were not found or are inactive.');
            }
            
            if (retrievalModelId && models.find(m => m.id === retrievalModelId)?.modelType !== 'RETRIEVAL') {
                throw new AppError('The selected Retrieval Model is not of type RETRIEVAL.');
            }
            if (generatorModelId && models.find(m => m.id === generatorModelId)?.modelType !== 'GENERATOR') {
                throw new AppError('The selected Generator Model is not of type GENERATOR.');
            }
        }
        ...
        if (Object.keys(updateData).length === 0) {
             throw new AppError('At least one model ID (retrievalModelId or generatorModelId) is required for update.');
        }

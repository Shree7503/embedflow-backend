...
export class QueryService {
  ...
  public async getRagConfiguration(project: Project): Promise<RagConfig> {
    ...
    if (!vectorDbCollectionName) {
        throw new AppError( `Project ${projectId} is missing a vector database collection name.`);
    }
    ...
    if (!retrievalModel) {
      throw new AppError( `Retrieval model (ID: ${retrievalModelId}) not found for project ${projectId}.`);
    }

    if (!generatorModel) {
      throw new AppError( `Generator model (ID: ${generatorModelId}) not found for project ${projectId}.`);
    }

    if (retrievalModel.modelType !== 'RETRIEVAL' || generatorModel.modelType !== 'GENERATOR') {
        throw new AppError(`Models assigned to project ${projectId} have incorrect types.`);
    }
    ...
    public async retrieveContext(userQuery: string, config: RagConfig): Promise<RetrievedChunk[]> {
        ...
        if (contextChunks.length === 0) {
            console.warn(`No context found for query in collection: ${config.vectorCollectionName}`);
        }

        return contextChunks;
    }
    
    private generateRagPrompt(userQuery: string, contextChunks: RetrievedChunk[]): string {
...
User Query: ${userQuery}`;
    ...
    public async generateResponse(userQuery: string, contextChunks: RetrievedChunk[], config: RagConfig): Promise<FinalQueryResponse> {
        
        const prompt = this.generateRagPrompt(userQuery, contextChunks);
        
        const generatedResponse = await llmApiUtility.generateResponse(config.generatorModel, prompt);

...
export class LLMApiUtility {
    ...
    public async generateEmbedding(modelConfig: Model, text: string): Promise<number[]> {
        ...
        try {
            ...
            throw new AppError( `Embedding generation failed for model ${modelConfig.modelName}.`);
        }
    }

   
    public async generateResponse(modelConfig: Model, prompt: string): Promise<string> {
        ...
        try {
            ...
            throw new AppError(`Response generation failed for model ${modelConfig.modelName}.`);
        }
    }
}
